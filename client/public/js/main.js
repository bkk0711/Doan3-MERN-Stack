...
CRYPTOKI.preload = function () {
    ...
    body.classNameList.remove('loading');
};
...
CRYPTOKI.mobileNav = function () {
    ...
    const open = function () {
        mobileNavPanel.classNameList.add('open');
        body.classNameList.add('body-overlay');
    }

    const close = function () {
        mobileNavPanel.classNameList.remove('open');
        body.classNameList.remove('body-overlay');
    }
    ...
    if (burgerIcon !== null) {
        burgerIcon.addEventListener('click', function () {
            if (mobileNavPanel !== null) {
                open();
                if (closeIcon !== null && mobileNavPanel.classNameList.contains('open')) {
        ...
        for (let i = 0; i < mobileMenuItem.length; i++) {
            mobileMenuItem[i].addEventListener('click', function (e) {
                ...
                if (currentSubMenu != null) {
                    currentSubMenu.classNameList.toggle('show');
                }
            });
        }
    }
...
CRYPTOKI.mobileSearch = function () {
    ...
    if (searchButton) {
        ...
        const show = function () {
            searchButton.classNameList.add('clicked');
            mobileSearchBlock.classNameList.add('is-visible');
            searchIconAttr.setAttributeNS(SVG_XLINK, 'xlink:href', '#cross-icon');
        }

        const hide = function () {
            searchButton.classNameList.remove('clicked');
            mobileSearchBlock.classNameList.remove('is-visible');
        ...
        if (searchButton !== null) {
            searchButton.addEventListener('click', function () {
                if (!mobileSearchBlock.classNameList.contains('is-visible')) {
                    show();
                } else {
                    hide();
                }
...
CRYPTOKI.mobileProfilePanel = function () {
    ...
    if (mediaQuery.matches) {
        ...
        const open = function () {
            if (!profilePanel.classNameList.contains('open')) {
                profilePanel.classNameList.toggle('open');
                profileAvatar.classNameList.add('hide');
                closingIcon.classNameList.add('show');
            }
        }

        const close = function () {
            if (profilePanel.classNameList.contains('open')) {
                profileAvatar.classNameList.remove('hide');
                closingIcon.classNameList.remove('show');
                profilePanel.classNameList.remove('open');
            }
        }
...
CRYPTOKI.socialCarousel = function () {
    ...
    if (socialCarousel !== null) {
        var mySwiper = new Swiper(socialCarousel, {
            ...
            slideVisibleclassName: 'swiper-slide-visible',
            loop: true,
            navigation: {
...
CRYPTOKI.tabs = function () {
    ...
    function tabsClick(tabClickEvent) {
        for (let i = 0; i < myTabs.length; i++) {
            myTabs[i].classNameList.remove('active');
        }
        let clickedTab = tabClickEvent.currentTarget;
        clickedTab.classNameList.add('active');
        tabClickEvent.preventDefault();
        let myContentTabs = document.querySelectorAll('.tab');
        for (let i = 0; i < myContentTabs.length; i++) {
            myContentTabs[i].classNameList.remove('active');
        }
        ...
        if (activeTab === null) {
            ...
            activeTab.classNameList.add('active');
        }
...
CRYPTOKI.accordion = function () {
    ...
    headings.forEach((h, i) => {
        ...
        btn.onclick = () => {
            ...
            plusIcon.classNameList.toggle('show');
            if (expanded) {
    ...
    function closeAllExpandedItems() {
        ...
        const expandedCopy = Array.from(accordionContents).filter((c) =>
            c.classNameList.value.includes(copyOpenclassName)
        );
        expandedTriggers.forEach((trigger) => {
            trigger.setAttribute("aria-expanded", false);
            const plusIcon = trigger.querySelector('.plus-icon');
            plusIcon.classNameList.add('show');
        });
        expandedCopy.forEach((copy) => {
            copy.classNameList.remove(copyOpenclassName);
    ...
    function closeItem(target, btn) {
        ...
        target.classNameList.remove(copyOpenclassName);
        target.style.maxHeight = 0;
    }
    function openItem(target, btn) {
        closeAllExpandedItems();
        btn.setAttribute("aria-expanded", true);
        target.classNameList.add(copyOpenclassName);
...
CRYPTOKI.headerDropDownsOnCLick = function () {
    ...
    if (elems !== null) {
        elems.forEach(function (el) {
            el.addEventListener('click', function (e) {
                ...
                if (_this.hasAttribute('data-target')) {
                    ...
                    terget_el.classNameList.toggle('active');
                    _this.classNameList.toggle('active');
        ...
        document.onclick = function (e) {
            if (e.target.closest('.cryptoki-notif-bttn') === null && !e.target.classNameList.contains('.cryptoki-notif-bttn') && e.target.closest('.cryptoki-notif-target') === null && !e.target.classNameList.contains('.cryptoki-notif-target')) {
                let targets = document.querySelectorAll(".cryptoki-notif-target, .cryptoki-notif-bttn");
                if (targets !== null) {
                    targets.forEach(function (el) {
                        el.classNameList.remove('active');
        ...
        if (lists !== null) {
            lists.forEach(function (el) {
                el.addEventListener('click', function (e) {
                    ...
                    _this.closest('.main-search').querySelector('.search-params').classNameList.remove('active');
...
CRYPTOKI.darkMode = function () {
    ...
    if (button !== null) {
        ...
        button.addEventListener('change', function (e) {
            document.body.classNameList.remove("light-mode", "dark-mode");

            if (!e.target.checked) {
                document.body.classNameList.add("light-mode");
                var theme = document.body.classNameList.contains("light-mode") ? "light" : "dark";
            } else {
                document.body.classNameList.add("dark-mode");
                var theme = document.body.classNameList.contains("dark-mode") ? "dark" : "light";
            }
